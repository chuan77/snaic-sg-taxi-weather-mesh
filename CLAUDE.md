...
| `hooks/use*.ts` | Data fetching hooks — each polls its JSON file every 5 min (useNowcast, useHotspots, useTaxis, useSurge, useClusters, useForecast, useSubzones, useWindowTaxis, useSubzoneCounts) |
...
| `components/TaxiClusterPage.tsx` | Subzone choropleth map — 30-min rolling window, GeoJSON fill + count badges + hover tooltips + top-20 table |
| `types/index.ts` | All shared TypeScript types (incl. `SubzoneProperties`, `SubzoneFeature`, `SubzoneCollection`) |
...
- **Data source:** `taxis_window.json` (30-min rolling window of distinct positions, generated by `taxi_window_export`) — not the current snapshot. Each of Singapore's ~3,000 taxis moves to ~5 different positions per 30 min, producing ~15,000 records. Airport and transit hubs show legitimately high counts.
- **GeoJSON file:** `web-dashboard/public/sg_subzones.geojson` — intentionally placed **outside** `public/data/` to avoid being shadowed by the Docker named volume (`public_data_vol:/app/web-dashboard/public/data`). Named volumes start empty and would make the file inaccessible at runtime.
- **Point-in-polygon (PIP):** Ray-casting algorithm in `useSubzoneCounts.ts` — handles `Polygon` and `MultiPolygon`, outer ring + hole exclusion. GeoJSON coordinates are `[lng, lat]` (reversed from Leaflet's `[lat, lng]`).
- **Leaflet height:** `MapContainer` requires a concrete pixel height. The page uses absolute positioning (`inset: 0`; map div `bottom: 160px`; table div `height: 160px`) instead of flex, which doesn't resolve to a concrete height reliably.
...
| `taxis_window.json` | `{ window_minutes: 30, total: number, taxis: [{lat, lng}] }` — distinct positions in the last 30 min |
